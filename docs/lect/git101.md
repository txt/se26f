...
First, the team mechanics: it is one team, one fork. ONE member
forks the past project; on that fork, Settings &rarr; Collaborators
adds the other three; everyone clones that one fork. Four separate
forks means four separate histories — and tutors read one history.
...
4. Never commit a secret. No keys. No passwords. A `.gitignore` lists
   ...
   For the Python forks of this course, that means at least:
