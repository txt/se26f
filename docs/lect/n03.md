...
## 0. Famous requirements failures ▪▪▪▪▪▪
...
## 1. Functional, non-functional, and constraints ▪▪▪▪▪▪▪▪
...
## 2. Stakeholders: who matters, who lies, who gets hurt ▪▪▪▪▪▪▪▪▪▪
...
## 3. Research corner: computing the NFR trade-offs
...
frontier is today's seed population.
...
1. **RE as theorem proving** (Zave & Jackson, 1997). The classic
   ...
   *S* such that
   ...
   — the assumptions plus your spec must entail the requirements.
   Requirements engineering as logic: one right answer, prove it.
2. **The requirements problem, revisited** (Jureta, Mylopoulos and
   colleagues, c. 2008–2012). Real requirements come with
   preferences, priorities, optional goals, and conflicts. The
   mandatory goals *G* and quality constraints *Q* must still be
   entailed:

   $$D, S \vdash G \wedge Q$$

   but many candidate *S* now qualify, so the answer is whichever
   candidate the stakeholders' preferences rank best:

   $$S^{*} = \arg\max_{S}\ \mathit{pref}(S)$$

   Then there is no single *S* to prove — there is a space of
   candidate solutions to search, and "solve" means find the best
   trade-offs. Proof becomes optimization.
3. **Abduction.** How do you generate those candidates? Abductive
   inference (Poole's framework): given *D* and a desired *R*,
   hypothesize a set of assumptions *A* that covers the goal while
   staying consistent:
   ...
   Each consistent set of assumptions is a
...
## 4. Project 1b launch: twelve prompts and the pivot question ▪▪▪▪▪▪
