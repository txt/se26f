...
backwards. A centrifuge test would have caught it. The test was
...
- **Repertory grids** — from psychology: take three things your
  ...
  emerges. (This is a research tool too; ask me how we use it.)
...
And real Pareto fronts hold hundreds of solutions — too many to
inspect. Two escapes:

- **Cluster the solutions** by which decisions they share (Veerappa &
  Letier): decide between a few groups first, then pick within the
  winning group.
- **Find the keys** (Mathew et al.): in model after model, a small
  set of decisions — typically ~12% — controls all the rest. Fix
  the keys and the tangle collapses; optimization runs 100 to 1000
  times faster, and stakeholder debate shrinks to the few decisions
  that matter.
