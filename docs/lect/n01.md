...
```
git switch -c feature-x     # make a branch, start work
git add -A; git commit      # commit on the branch, as usual
git diff main...feature-x   # what did this branch change, vs main?
git switch main
git merge feature-x         # bring the work into main
```

For team work, do not merge locally. Push the branch and open a pull
request:
...
branch, sharing one repo:

```
git worktree add ../wt-a1 -b agent1    # a sandbox for agent 1
# ... agent works in ../wt-a1 while you work elsewhere ...
git diff main...agent1                 # review the agent's work
git merge --no-ff agent1               # accept it into your main
git worktree remove ../wt-a1           # clean up
```
